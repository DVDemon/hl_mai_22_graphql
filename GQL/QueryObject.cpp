...
service::AwaitableResolver Query::resolveAuthor(service::ResolverParams&& params) const
{
	auto argId = service::ModifiedArgument<int>::require<service::TypeModifier::Nullable>("id", params.arguments);
	std::unique_lock resolverLock(_resolverMutex);
	auto directives = std::move(params.fieldDirectives);
	auto result = _pimpl->getAuthor(service::FieldParams(service::SelectionSetParams{ params }, std::move(directives)), std::move(argId));
	resolverLock.unlock();

	return service::ModifiedResult<Author>::convert<service::TypeModifier::Nullable>(std::move(result), std::move(params));
}

service::AwaitableResolver Query::resolveAllAuthors(service::ResolverParams&& params) const
{
	std::unique_lock resolverLock(_resolverMutex);
	auto directives = std::move(params.fieldDirectives);
	auto result = _pimpl->getAllAuthors(service::FieldParams(service::SelectionSetParams{ params }, std::move(directives)));
	resolverLock.unlock();
...
{
	typeQuery->AddFields({
		schema::Field::Make(R"gql(author)gql"sv, R"md()md"sv, std::nullopt, schema->LookupType(R"gql(Author)gql"sv), {
			schema::InputValue::Make(R"gql(id)gql"sv, R"md()md"sv, schema->LookupType(R"gql(Int)gql"sv), R"gql()gql"sv)
		}),
		schema::Field::Make(R"gql(allAuthors)gql"sv, R"md()md"sv, std::nullopt, schema->WrapType(introspection::TypeKind::LIST, schema->LookupType(R"gql(Author)gql"sv))),
		schema::Field::Make(R"gql(search)gql"sv, R"md()md"sv, std::nullopt, schema->WrapType(introspection::TypeKind::NON_NULL, schema->WrapType(introspection::TypeKind::LIST, schema->WrapType(introspection::TypeKind::NON_NULL, schema->LookupType(R"gql(Author)gql"sv)))), {
